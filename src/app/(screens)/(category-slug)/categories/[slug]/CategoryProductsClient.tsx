...
const CategoryProductsClient: React.FC<Props> = ({ slug }) => {
  ...
  const fetchProducts = async () => {
    ...
    try {
      const data = await getProductsFilter(
        `?search=${search || ""}&slug=${slug}&brand=${brand}&industry=${industry}${
          sortUrl ? sortUrl : ""
  ...
  useEffect(() => {
    ...
    fetchSeo();
  }, [slug, search, brand, industry, sortType, page, limit]);
      ...
      <div className={`mobile-sidebar ${sidebarOpen ? "show" : ""}`}>
        ...
        <div className="p-3">
          <PermanentFilterSidebar
            setCategory={() => {}} // Empty function since we don't need category filter
            setBrand={setBrand}
            ...
            hideCategory={true} // Hide category filter
          />
        </div>
      </div>
