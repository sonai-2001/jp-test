...
interface ManufacturersNameProps {
  heading?: string;
  button?: boolean;
}
function BrandsSection({ heading = "Our Trusted Brands",button=false}: ManufacturersNameProps) {
            ...
            {getBrandList?.length ? (
              <>
                <EmblaCarousel option={option} displayButtons={false}>
                  {getBrandList
                    ?.filter((fItem: any) => fItem?.brandImage)
                ...
                {button ? (
                  <>
                    <Box mt={3} textAlign="center">
                      <Button onClick={() => router.push("/brands")} variant="contained" color="primary" size="medium">
                        VIEW ALL
                      </Button>
                    </Box>
                  </>
