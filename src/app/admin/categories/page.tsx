...
import { deleteCategory, getCategories } from "@/app/services/Category/CategoryApi";
...
function CategoryFormList() {
  ...
  const onSubmitDelete = async () => {
    try {
      const response = await deleteCategory(getSectorDelete?.id);
      if (response.ok) {
        setSectorCloseModel();
      }
    } catch (error) {
      ...
      >
        ...
        <Modal.Body className="modal-body">
          <div className="deadline-form">
            ...
            >
              Are you sure to delete "{getSectorDelete?.category}" Category?
            </h4>
